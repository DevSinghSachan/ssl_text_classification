...
# Percentage of Pre-trained word vectors: 68.42837475545251
# Create pre-trained word
python w2v.py --input ${OUT}/data --save_data "demo_fastText" --embeddings "${HOME}/crawl-300d-2M.vec"

exit
# Train the model
PYTHONIOENCODING=utf-8 python main.py --corpus rcv1 --model LSTMEncoder --debug --save_data "demo_fastText" \
--multi_gpu --input ${OUT}/data --output_path ${OUT}/model --exp_name "rcv1_clf_" \
