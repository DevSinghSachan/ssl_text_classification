...
def work(id, queue):
    ...
    while True:
        ...
        arg_string = \
        'env CUDA_VISIBLE_DEVICES={} PYTHONIOENCODING=utf-8 python main.py --corpus imdb --model LSTMEncoder '\
        '--debug --save_data demo_fastText '\
        '--multi_gpu --input temp/imdb_pretrained/data --output_path temp/imdb_pretrained/model '\
        '--exp_name imdb_clf_mixed_{} '\
        '--use_pretrained_embeddings --nepoch_no_imprv 20 --timedistributed --d_hidden 512 --nepochs 40 --optim adam '\
        '--wbatchsize 10000 --wbatchsize_unlabel 10000 --eval_steps 1000 --lstm_dropout 0.5 --word_dropout 0.5 '\
        '--beta1 0.0 --num_layers 1 --beta2 0.98 --scheduler ExponentialLR --gamma 0.9999998 '\
        '--perturb_norm_length {} --lambda_entropy 1.0 --lambda_vat 1.0 --lambda_at 1.0 ' \
        '--inc_unlabeled_loss --unlabeled_loss_type AvgTrainUnlabel'
...
perturb_norm_length_list = [1, 2, 3, 4]
